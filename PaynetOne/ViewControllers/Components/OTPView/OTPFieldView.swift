...
@objc public class OTPFieldView: UIView {
    ...
    public var fieldFont: UIFont = .helvetica.withSize(28)
    public var secureEntry: Bool = false
    public var hideEnteredText: Bool = false
    public var requireCursor: Bool = true
    public var cursorColor: UIColor = .blueColor
    public var fieldSize: CGFloat = 40
    ...
    public var defaultBorderColor: UIColor = .blueColor
    public var filledBorderColor: UIColor = .blueColor
    public var errorBorderColor: UIColor?
    var otpViewWidth: CGFloat = screenWidth - 50
    var otpViewHeight: CGFloat = 100
    var secureEntryFont: UIFont = .helvetica.withSize(40)
    ...
    fileprivate func getOTPField(forIndex index: Int) -> OTPTextField {
        ...
        if hasOddNumberOfFields {
            ...
            fieldFrame.origin.x = otpViewWidth / 2 - (CGFloat(fieldsCount / 2 - index) * (fieldSize + separatorSpace) + fieldSize / 2)
        }
        else {
            // Calculate from middle each fields x and y values so as to align the entire view in center
            fieldFrame.origin.x = otpViewWidth / 2 - (CGFloat(fieldsCount / 2 - index) * fieldSize + CGFloat(fieldsCount / 2 - index - 1) * separatorSpace + separatorSpace / 2)
        }
        
        fieldFrame.origin.y = (otpViewHeight - fieldSize) / 2
    ...
    public func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if replacedText.count >= 1 {
            ...
            if hideEnteredText {
                ...
                if secureEntry {
                    textField.text = "•"
                    textField.font = secureEntryFont
